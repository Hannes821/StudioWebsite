...
test('all generated internal links resolve, including PDFs and heading anchors',async()=>{
 ...
 for(const match of html.matchAll(/(?:href|src)="([^"]+)"/g)){const ref=match[1];if(/^(https?:|mailto:)/.test(ref))continue;const [file,anchor]=ref.split('#');const target=file?decodeURIComponent(file).replace(/^\/?wiki\//,'').replace(/^\//,''):name;assert.ok(!target.includes('..'));let data;try{data=await fs.readFile(new URL(target,root));}catch{assert.fail(`${name}: missing ${target}`);}if(anchor&&target.endsWith('.html'))assert.ok(data.toString().includes(`id="${anchor}"`),`${name}: missing #${anchor}`);count++;}}
 assert.ok(count>100);
});
test('search index contains all articles and key concepts',async()=>{const index=JSON.parse(await fs.readFile(new URL('search.json',root),'utf8'));assert.equal(index.length,31);for(const term of ['HealthComponent','Piercing','Middle High German','Gameplay Tags'])assert.ok(index.some(p=>(p.text+p.title).includes(term)),term);});
